...
if __name__ == "__main__":
    ...
    OBJECTS = [
        ("/Volumes/One Touch/models library/Creative_Models/v1-5-pruned-emaonly.safetensors",
         ["cond_stage_model.transformer.text_model.embeddings.token_embedding.weight",
          "model.diffusion_model.input_blocks.7.1.transformer_blocks.0.ff.net.0.proj.weight"],
         "SD1.5"),
        ("/Volumes/One Touch/models library/Creative_Models/sd_xl_base_1.0.safetensors",
         ["conditioner.embedders.1.model.token_embedding.weight",
          "model.diffusion_model.input_blocks.7.1.transformer_blocks.0.ff.net.0.proj.weight"],
         "SDXL"),
    ]
    for path, keys, label in OBJECTS:
        f = safe_open(path, framework="numpy")
        for k in keys:
            probe(f"{label} {k.split('.')[-2]}.{k.split('.')[-1]} {f.get_slice(k).get_shape()}", f.get_tensor(k))
    import glob, os
    ...
    if snaps:
        ...
        for _, k in mats:
            probe(f"Kokoro {k}", flat[k].float().numpy())
    print("\nRUNG 1 (VALIDATED OBJECTS) COMPLETE", flush=True)
