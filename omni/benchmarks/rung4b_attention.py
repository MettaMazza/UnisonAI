...
def run(seed):
    ...
    print(f"CASCADE seed={seed}  val={vl:.4f}  ({time.time()-t0:.0f}s)", flush=True)
    return vl

if __name__ == "__main__":
    C = [run(s) for s in SEEDS]
    print(f"\nBASELINE (recorded, trained softmax): 1.8878", flush=True)
    print(f"CASCADE mean val loss: {np.mean(C):.4f}", flush=True)
    print("VERDICT:", "CASCADE WINS/TIES -- rung taken" if np.mean(C) <= 1.8878 + 0.005
          else "softmax baseline wins -- recorded", flush=True)
    print("RUNG 4b COMPLETE", flush=True)
